...
def merge_weather_data(train, test, weather_train_df, weather_test_df):
...
def plot_meter_measurements_vs_floor_count(df:pd.DataFrame):
    fig, axes = plt.subplots(2, 2, figsize=(14, 6))
    for i in df['meter'].value_counts(dropna=False).index.to_list():
        name_of_meter = METER_DIC[i]
        pd.Series(index=df[df['meter'] == i]['floor_count'].value_counts().index,
                  data=df[df['meter'] == i].groupby('floor_count')['meter_reading'].transform(
                      'mean').value_counts().index).sort_index().plot(kind='bar', rot=0, ax=axes[i % 2][i // 2])
        axes[i % 2][i // 2].set_xlabel('Floor count')
        axes[i % 2][i // 2].set_ylabel('Mean {} meter reading'.format(name_of_meter))
        axes[i % 2][i // 2].set_title('Mean {} meter reading by floor count'.format(name_of_meter))
    fig.savefig(cfg.eda_dir + "meter_vs_floor.png")
    fig.show()
...
def plot_temperature_vs_time_according_to_site(train, test):
    fig, axes = plt.subplots(8, 2, figsize=(14, 30), dpi=100)
    for i in range(train['site_id'].nunique()):
        train[train['site_id'] == i][['timestamp', 'air_temperature']].set_index('timestamp').resample('H').mean()[
            'air_temperature'].plot(ax=axes[i % 8][i // 8], alpha=0.8, label='By hour', color='tab:blue').set_ylabel(
            'Mean temperature', fontsize=13)
        test[test['site_id'] == i][['timestamp', 'air_temperature']].set_index('timestamp').resample('H').mean()[
            'air_temperature'].plot(ax=axes[i % 8][i // 8], alpha=0.8, color='tab:blue', label='').set_xlabel('')
        train[train['site_id'] == i][['timestamp', 'air_temperature']].set_index('timestamp').resample('D').mean()[
            'air_temperature'].plot(ax=axes[i % 8][i // 8], alpha=1, label='By day', color='tab:orange')
        test[test['site_id'] == i][['timestamp', 'air_temperature']].set_index('timestamp').resample('D').mean()[
            'air_temperature'].plot(ax=axes[i % 8][i // 8], alpha=1, color='tab:orange', label='').set_xlabel('')
        axes[i % 8][i // 8].legend()
        axes[i % 8][i // 8].set_title('site_id {}'.format(i), fontsize=13)
        axes[i % 8][i // 8].axvspan(test['timestamp'].min(), test['timestamp'].max(), facecolor='green', alpha=0.2)
        plt.subplots_adjust(hspace=0.45)
    fig.savefig(cfg.eda_dir + "temperature_in_each_site_according_time.png")
    fig.show()
...
def fix_time_adjustment(weather_train:pd.DataFrame, weather_test:pd.DataFrame):
    ...
    #  air temperature mean over the days according to the site_id
    # tmp_df['temp_rank'] = tmp_df.groupby(['site_id', tmp_df.timestamp.dt.date])['air_temperature'].rank('average')
    # #  creating a data frame that holds the mean of temp_rank
    # #  according to hours(which is equal to the number of samples) and site_id
    # #  we then unstack it to make the hours as the columns and
    # #  the site_id as the the rows, the elements of this matrix is temp_rank value
    # tmp_df2 = tmp_df.groupby(['site_id', tmp_df.timestamp.dt.hour])['temp_rank'].mean().unstack(level=1)
    # #  we arrange the offset needed for each site according to the most common peak of temperature (which is the 14 hour)
    # site_id_offsets = pd.Series(tmp_df2.values.argmax(axis=1) - 14)
    # site_id_offsets.index.name = 'site_id'
    # #  next we align all the timestamps according to the offset
    # tmp_df['offset'] = tmp_df.site_id.map(site_id_offsets)
    # tmp_df['timestamp_aligned'] = tmp_df.timestamp - pd.to_timedelta(tmp_df.offset, unit='H')
    # tmp_df.drop(['offset', 'temp_rank'], inplace=True, axis='columns')
    # # train_ans = tmp_df[tmp_df['DataType'] == 'train']
    # # test_ans = tmp_df[tmp_df['DataType'] == 'test']
    # weather_train['timestamp'] = tmp_df[tmp_df['DataType'] == 'train']['timestamp_aligned']
    # weather_test['timestamp'] = tmp_df[tmp_df['DataType'] == 'test']['timestamp_aligned']
    # weather_train.drop(['DataType'], inplace=True, axis='columns')
    # weather_test.drop(['DataType'], inplace=True, axis='columns')
...
if __name__ == '__main__':
    ...
    print(test_df.shape)
    # temp_plot(train_df)
    weather_train_df, weather_test_df = fix_time_adjustment(weather_train_df, weather_test_df)
    print(train_df.shape)
    print(test_df.shape)
